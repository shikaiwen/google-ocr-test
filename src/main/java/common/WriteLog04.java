 ...
 * LOG
 */
public class WriteLog04 {

	private BufferedWriter success_output = null;

	private BufferedWriter faild_output = null;

	public WriteLog04(String folder) {
		String fileName = CommonUtil.getCurrentDateTime().substring(0, 10).replace("/", "") + ".log";
		try {
			File success_LogFilePath = new File(Constants.LOG_ROOT_PATH_MAIL + folder);
			File faild_LogFilePath = new File(Constants.LOG_ROOT_PATH_MAIL + folder);
			File success_LogFile = new File(Constants.LOG_ROOT_PATH_MAIL + folder + "\\analysis_success-" + fileName);
			File faild_LogFile = new File(Constants.LOG_ROOT_PATH_MAIL + folder + "\\analysis_faild-" + fileName);
	 ...
	 * @return
	 */
    public void writeLog04(String content, Boolean flag) {
